...
import {CirclesContext} from './CirclesContext';
...
const EventCard = ({ name }) => {
  ...
  return (
    <Animated.View
      style={[styles.eventCircle, { transform: [{ translateY }] }]}
    >
      ...
      <TouchableOpacity
        onPress={() => {
          if (!canAccept) {
            Alert.alert(
              "Max Circles Reached",
              "You have reached the maximum number of circles."
            );
          } else {
            // Voeg hier de logica toe om de uitnodiging te accepteren
            Alert.alert(
              "Invitation Accepted",
              `You have accepted the invitation to join ${name}.`
            );
          }
        }}
      >
