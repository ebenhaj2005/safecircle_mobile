...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://10.2.88.155:8080/user/authenticate', { // IP-adres van je thuis wifi
        method: 'POST',
        headers: {
  ...
  const refreshTokens = async () => {
    try {
      const response = await fetch('http://10.2.88.155:8080/user/refresh-token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
